...
export default function MyTestsWidget(){
  ...
  const Badge = ({s}:{s:Row["status"]})=>{
    ...
    const t = s==="not_started"?"ikke startet":s==="in_progress"?"pågår":"fullført"
    return <span className={`px-2 py-1 rounded text-xs font-medium ${m[s]}`}>{t}</span>
  }

  return (
    <div className="space-y-3">
      {rows.map(t=>(
        <div key={t.test_id} className="border rounded p-3 flex items-center justify-between gap-4">
          <div className="min-w-0">
            <div className="font-medium truncate">{t.template_name}</div>
            <div className="text-xs text-gray-500">{t.answered}/{t.total_items} besvart</div>
            <div className="mt-2 h-2 bg-gray-100 rounded">
              <div className="h-2 bg-emerald-500 rounded" style={{width:`${Math.round(t.progress*100)}%`}}/>
            </div>
          </div>
          <div className="flex items-center gap-3 shrink-0">
            <Badge s={t.status}/>
            {t.status==="complete" ? (
              <button onClick={()=>nav(t.links.scores)}
                      className="px-3 py-2 rounded bg-slate-100 hover:bg-slate-200 text-slate-800">
                Vis skårer
              </button>
            ) : (
              <button onClick={()=>nav(t.links.continue)}
